...
struct termios TERMINAL_STDIN_DEFAULTS;
...
void dbg_dump_st_contents ( const struct file* fil ) {
...
// helper fn, does what it says it does (moves cur line into previous one) and deletes the prev line
// places cursor at the end of the previous line but before current line content
...
void global_term_init () {
	...
	tcgetattr( STDIN_FILENO, &TERMINAL_STDIN_DEFAULTS );
	memcpy( &attr, &TERMINAL_STDIN_DEFAULTS, sizeof( struct termios ) );
...
// 	so 1000 will come as 1,0,0,0, so for every new char we multiply old by 10)
	...
	while ( (c = __getchar()) != 'R' ) {		// 	row digits
		x *= 10;
		x += c - '0';
	}
...
void line_right ( struct term* tm, struct file* fil ) {
	int tmp_variable = tm->cur_col;
	tm->cur_col++;
	recompute_offsets_from_curcol( tm, fil, 0 );
// 							this is ghastly ik, runtime of c+n
	if ( tm->cur_col == tmp_variable ) {
		if ( fil->cur_ln == fil->amt - 1 ) {
			tm->cur_col = tmp_variable;
			return;
		}
		tm->cur_col = 0;
		line_down( tm, fil );
...
void line_left ( struct term* tm, struct file* fil ) {
	int tmp_variable = tm->cur_col;
...
void term_insert_char ( struct term* tm, struct file* fil, int c ) {
	if ( c >= 0x20 || c == '\t' ) {
		tm->cur_col++;
		f_insert_char( fil, c );
		recompute_offsets_from_curcol( tm, fil, 0 );
...
void handle_esc_seq ( struct term* tm, struct file* fil ) {
	char buffer[ 32 ];
	int length = 0;
	char c = 0;

	while ( ( c = __getchar() ) != 0 && length < 32 ) {
		buffer[ length ] = (char)c;
		length++;
	}

	term_insert_char( tm, fil, '^' );
	for ( int i = 0; i < length; i++ ) {
		term_insert_char( tm, fil, buffer[ i ] );
	}
...
// 					term_insert_char( tm, fil, '^' );
// 					term_insert_char( tm, fil, '[' );
// 					term_insert_char( tm, fil, '1' );
// 					term_insert_char( tm, fil, c );
// 				} else {
// 					c = __getchar();
// 					if ( c != '5' ) {
// 						switch( __getchar() ) {
...
void cleanup () {
	...
	tcsetattr( STDIN_FILENO, TCSANOW, &TERMINAL_STDIN_DEFAULTS );
...
int main ( int argc, char* argv[] ) {
	...
	struct file* fil = alloc_file( argv[ 1 ] );
	...
	*term_list = trm;
	...
	do {
		...
		switch ( c ) {
			case 0x1B:
				handle_esc_seq( trm, fil );
				break;
			case '\r':
			case '\n':
				f_enter( fil );
				trm->cur_col = 0;
				trm->cur_row = fil->cur_ln - trm->lin_off;
				check_ln_down( trm, fil );
				trm->cur_row = fil->cur_ln - trm->lin_off;
				break;
			case 0x7F:
// 				backspace
...
// 						if there are no more lines, we just stay where we are
				...
				f_delete_char( fil );
				recompute_offsets_from_curcol( trm, fil, 0 );
				break;
			case 0x08:
// 				case ctrl+backspace
			case 0:
